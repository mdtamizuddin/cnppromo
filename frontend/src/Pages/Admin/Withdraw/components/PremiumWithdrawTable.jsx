...
const PremiumWithdrawTable = () => {
  ...
  const { data, isLoading, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery(
    ["Premium Withdraw Table", status, option.limit, textSearch, dateSearch],
    async ({ pageParam = 1 }) => {
      let url = `/withdraw?page=${pageParam}&limit=${option.limit}&reverse=true&status=${status}`;
  ...
  const totalAmount = data?.pages[0]?.totalWithdraw || 0;
  ...
  return (
    <div className="w-full pb-10">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Manage withdrawal requests from users.</p>
      </div>
            ...
            <div key={withdraw._id} className="p-3.5 space-y-2.5 hover:bg-blue-50/20 transition-colors">
              ...
              <div className="flex items-center justify-between pt-1 text-xs">
                <div className="flex items-center gap-1.5">
                  <img src={logoProvider(withdraw.method?.toLowerCase())} alt={withdraw.method} className="w-4 h-4 object-contain" />
                  <span className="text-gray-700 font-medium">{withdraw.method}</span>
                  <span
                ...
                <span className="text-base font-black text-gray-900 font-mono">৳ {withdraw.amount}</span>
              </div>
          ...
          <div ref={ref} className="py-3 text-center">
            {isFetchingNextPage ? (
              ...
              <div className="flex flex-col items-center justify-center py-6">
                ...
                <span className="text-xs text-gray-500">No withdrawals found matching your criteria.</span>
              </div>
            )}
          </div>
        </div>
              ...
              {allWithdraws.map((withdraw, index) => {
                ...
                return (
                  <tr key={withdraw._id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm font-bold text-gray-900">৳ {withdraw.amount}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <img 
                        ...
                        />
                        <div className="flex flex-col">
                           <span className="text-xs font-semibold text-gray-900">{withdraw.method}</span>
                ...
                <td colSpan="6" className="py-4 text-center">
                  {isFetchingNextPage ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-10">
                      ...
                      <span className="text-sm text-gray-500">No withdrawals found matching your criteria.</span>
                    </div>
                  )}
                </td>
              </tr>
...
export default PremiumWithdrawTable;
