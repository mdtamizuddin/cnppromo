...
import ExternalWithdrawActionModal from "./ExternalWithdrawActionModal";
...
const PremiumExternalWithdrawTable = () => {
  ...
  const { data, isLoading, refetch, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery(
    ["Premium External Withdraw Table", status, option.limit, textSearch, dateSearch],
    async ({ pageParam = 1 }) => {
      let url = `/external-withdraw?page=${pageParam}&limit=${option.limit}&reverse=true&status=${status}`;
  ...
  const allWithdraws = data?.pages.flatMap((page) => page.data) || [];
  ...
  return (
    <div className="w-full pb-10">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl font-bold text-gray-900">External Withdrawals</h1>
        <p className="text-sm text-gray-500 mt-1">Manage external withdrawal requests containing video or image proof.</p>
      </div>
      ...
      <Card className="w-full shadow-sm border border-gray-200 overflow-hidden rounded-xl">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            <form onSubmit={searchHandler} className="w-full sm:w-64 relative">
              <Input
                name="search"
                type="text"
                placeholder="Search by username, email, account..."
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] table-auto text-left whitespace-nowrap">
            <thead>
              <tr>
                {["Date", "User", "Proof Type", "Method", "Status", "Action"].map((head) => (
            ...
            <tbody className="divide-y divide-gray-100">
              {allWithdraws.map((withdraw, index) => {
                const isLast = index === allWithdraws.length - 1;
                return (
                  <tr key={withdraw._id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="px-4 py-3">
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold text-gray-900">{moment(withdraw.createdAt).format("MMM DD, YYYY")}</span>
                        <span className="text-xs text-gray-500">{moment(withdraw.createdAt).format("hh:mm A")}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        />
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{withdraw.user?.name}</span>
                    ...
                    <td className="px-4 py-3">
                       {withdraw.video ? (
                         <span className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-full text-[11px] font-bold tracking-wide uppercase border border-indigo-100">Video</span>
                       ) : withdraw.image ? (
                         <span className="px-2.5 py-1 bg-cyan-50 text-cyan-700 rounded-full text-[11px] font-bold tracking-wide uppercase border border-cyan-100">Image</span>
                       ) : (
                         <span className="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-full text-[11px] font-bold tracking-wide uppercase border border-gray-200">None</span>
                       )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <div className="flex flex-col">
                           <span className="text-xs font-semibold text-gray-900 capitalize" title="Method">{withdraw.method}</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="w-max">
                        <Chip
                          ...
                          icon={
                            withdraw.status === "completed" ? <CheckCircleIcon className="w-4 h-4" /> :
                            withdraw.status === "rejected" ? <XCircleIcon className="w-4 h-4" /> :
                ...
                <td colSpan="6" className="py-4 text-center">
                  {isFetchingNextPage ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-10">
                      ...
                      <span className="text-sm text-gray-500">No requests found matching your criteria.</span>
                    </div>
                  )}
                </td>
              </tr>
...
export default PremiumExternalWithdrawTable;
