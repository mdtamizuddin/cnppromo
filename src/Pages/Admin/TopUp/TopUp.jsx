...
const TABLE_HEAD = ["Date", "Name", "Whatsapp", "Amount", "Payment",
    "Status", ""];


export default function Topups({ historyType }) {
    ...
    const { data, isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get(`/topup?page=${option.page}&limit=${option.limit}&${search && `search=${search}`}&status=${status}`);
            return res.data
        }
    })
    ...
    const actionHandler = async (id, type) => {
        try {
            if (type === "completed") {
                const res = await api.put(`/topup/accept/${id}`)
                toast.success(res.data.message)
                refetch()
            }
            else {
                const res = await api.put(`/topup/${id}`, {
                    status: type
                })
                toast.success(res.data.message)
                refetch()
        ...
        <Card className="h-full w-full container my-10 lg:p-0 p-5">
                ...
                <div className="mb-4 flex flex-col justify-between gap-8 md:flex-row md:items-center">
                    <div>
                        ...
                        <Typography color="gray" className="mt-1 font-normal capitalize">
                            Manage all Topup Request
                        </Typography>
                    </div>

                    <form onSubmit={queryFn} className="flex w-full shrink-0 gap-2 md:w-max">
            ...
            <CardBody className="overflow-scroll px-0">
                ...
                <table className="w-full  table-auto text-left">
                    ...
                    <tbody>
                        {data?.data?.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).map(
                            (
                                {
                                    _id,
                                    amount, status, method, account, createdAt,trx, user
                                },
                                index,
                            ) => {
                                    ...
                                    <tr key={_id}>
                                        ...
                                        <td className={classes}>
                                            <Typography
                                                ...
                                                className=""
                                            >
                                                <a href={`https://wa.me/${user.phone}`} target="_blank" rel="noreferrer" className="cursor-pointer hover:text-primary font-normal text-xs">
                                                    {user.phone}
                                                </a>
                                            </Typography>
                                        </td>
                                        ...
                                        <td className={classes}>
                                            <div className="flex items-center gap-3">
                                                <div className="h-9 w-12 rounded-md border border-blue-gray-50 p-1">
                                                    <Avatar
                                                        src={
                                                            logoProvider(method?.toLowerCase())
                                                        }
                                                        size="sm"
                                                        alt={method}
                                                        variant="square"
                                                ...
                                                <div className="flex flex-col">
                                                    ...
                                                    <Typography
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            toast.success("Transection Copyed")
                                                        }}
                                                    >
                                                        {trx}
                                                    </Typography>
            ...
            <CardFooter className="flex items-center justify-between border-t border-blue-gray-50 p-4">
                ...
                <div className="flex items-center gap-2">
                    <Pagination pages={data?.pages} setState={(e) => {
